...
const WhyUsSection = () => {
  ...
  useEffect(() => {
    if (sliderRef.current) {
      // Calculate the height of one complete set of images
      const singleSetHeight = imageHeight * imageUrls.length;
      setSliderHeight(singleSetHeight);
    }
  }, []);

  // Animation for the slider - continuous loop
  useEffect(() => {
    if (!sliderHeight) return;
    
    const slideSpeed = isHovered ? 0.5 : 1.2; // Slow down when hovered
    
    const animationFrame = requestAnimationFrame(function animate() {
      setSliderPosition(prev => {
        // When we've scrolled the height of one image, reset position to create seamless loop
        if (Math.abs(prev) >= imageHeight) {
          // Move the first image to the end
          const newPosition = prev + imageHeight;
          return newPosition;
        }
  ...
  }, [sliderHeight, isHovered, imageHeight]);
  ...
  const filmReelStyles = isDarkMode
    ? 'bg-gray-800/70 border-yellow-600/20 shadow-[inset_3px_3px_6px_rgba(0,0,0,0.5),inset_-2px_-2px_6px_rgba(255,255,100,0.03)]'
    : 'bg-yellow-100/80 border-yellow-400/30 shadow-[inset_5px_5px_10px_rgba(0,0,0,0.07),inset_-5px_-5px_10px_rgba(255,255,255,0.8)]';
          ...
          >
            ...
            >
              ...
              <div className="absolute inset-0 overflow-hidden mx-6">
                ...
                >
                  {/* We duplicate the images multiple times to ensure continuous looping */}
                    ...
                    imageUrls.map((src, imgIndex) => (
                      ...
                      >
                        <div 
                          ...
                          `}
                        >
                          {/* Image frame with slight grain overlay */}
