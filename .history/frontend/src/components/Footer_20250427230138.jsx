  ...
  return (
    <div className={`mt-14 rounded-t-3xl flex justify-center relative overflow-hidden
      ${isDarkMode 
        ? ' text-blue-100' 
        : ' text-blue-900'}`}>
<div className="w-3/6 h-0.5  bg-blue-950 rounded-full absolute backdrop-brightness-90"></div>
              ...
              <div className="py-8 px-4">
                ...
                <ul className="flex flex-col gap-3">
                  {FooterLinks.map((link, index) => (
                    <li key={index} className="group cursor-pointer transition-all duration-300">
                      <a 
                        href={link.link}
                        className={`flex items-center gap-2 group-hover:translate-x-1 transition-all
                          ${isDarkMode ? 'text-blue-100/70' : 'text-blue-800/70'} 
                          group-hover:${isDarkMode ? 'text-yellow-300' : 'text-green-600'}`}>
                        <FaArrowRight className={`text-xs transition-transform group-hover:rotate-90
